...
export default function UniformStatusPage() {
    const token = "123"
    ...
    const visibleData = selectedFilter
        ? homeworkStatusData.filter((row) => {
            ...
            if (selectedFilter === "Uniform") return status === 1;
            if (selectedFilter === "Ununiform") return status === 0;
            if (selectedFilter === "Permission") return status === 2;
            return false;
        })
        : homeworkStatusData;
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setHomeworkDoneSaved(res.data.dressCodeSaved);
            setHomeworkStatusData(res.data.dresscodeStatusDetails)
    ...
    useEffect(() => {
        if (homeworkStatusData?.length) {
            ...
            homeworkStatusData.forEach((item) => {
                ...
                mappedStatuses[item.rollNumber] = status;
                mappedComments[item.rollNumber] = item.remarks || "";
            });
            ...
            <Box sx={{ marginTop: "-10px", px: 2 }}>
                <Box sx={{ display: "flex" }}>
                    <Grid container sx={{ justifyContent: "space-between" }}>
                        ...
                        <Grid item xs={12} sm={12} md={5} lg={9} sx={{ display: "flex", justifyContent: "end" }} >
                            <Button
                                variant="outlined"
                            ...
                            >
                                <Box sx={{ p: 2 }}>
                                    <Typography sx={{ pb: 2, fontWeight: "600", fontSize: "18px" }}>Export Homework Completion Status</Typography>
                                ...
                                <TableRow key={row.id}>
                                    ...
                                    <TableCell sx={{ borderRight: 1, borderColor: "#E8DDEA", width: "200px" }}>
                                        <Autocomplete
                                            ...
                                            options={["Uniform", "Ununiform", "Permission"]}
                                            value={
                                                statuses[row.rollNumber] === 1
                                                    ? "Uniform"
                                                    : statuses[row.rollNumber] === 0
                                                        ? "Ununiform"
                                                        : statuses[row.rollNumber] === 2
                                                            ? "Permission"
                                                            : null
                                            }
                                            onChange={(e, newValue) => {
                                                let mappedValue;
                                                if (newValue === "Uniform") mappedValue = 1;
                                                else if (newValue === "Ununiform") mappedValue = 0;
                                                else if (newValue === "Permission") mappedValue = 2;
                                                else mappedValue = null;
                                            ...
                                            renderInput={(params) => {
                                                const status = statuses[row.rollNumber];
                                                let inputColor = "inherit";
                                                if (status === 1) inputColor = "green";
                                                else if (status === 0) inputColor = "red";
                                                else if (status === 2) inputColor = "blue";
                                    ...
                                    >
                                        <Button
                                            ...
                                            sx={{ py: 0.1, textTransform: "none", borderRadius: "30px", border: "1px solid #A9A9A9", color: "#000" }}
                                        >
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    />
                                                    <Typography variant="caption" sx={{ color: "#777", textAlign: "right", display: "block", mb: 1 }}>
                                                        {comments[row.rollNumber]?.length || 0} /100
                                                    </Typography>
